    ...
    <header className={`site-header ${scrolled ? 'bg-parchment/90 backdrop-blur supports-[backdrop-filter]:bg-parchment/70' : 'bg-parchment'}`}>
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
        <Link href="/" className="flex-1 min-w-0 flex items-center gap-2 no-underline">
          ...
          <span className="font-display text-xl tracking-tight text-forest shrink-0 font-semibold">The Anchor</span>
        ...
        <nav className="hidden md:flex items-center gap-6">
          {links.map(l => (
            <Link
              key={l.href}
              href={l.href}
              className={`text-[15px] no-underline ${pathname === l.href ? 'active' : 'text-timber/90 hover:text-forest font-medium'}`}
              aria-current={pathname === l.href ? 'page' : undefined}
            >
              {l.label}
            </Link>
          ))}
          <a href="/#booking" className="btn btn-cta no-underline rounded-2xl">Book Your Stay</a>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-stone/60 bg-parchment">
          <nav className="px-4 py-3 flex flex-col gap-2">
            {links.map(l => (
              <Link key={l.href} href={l.href} className="py-2 text-timber font-medium">{l.label}</Link>
            ))}
            <a href="/#booking" className="btn btn-cta rounded-2xl no-underline mt-2">Book Your Stay</a>
          </nav>
        </div>
      )}
    </header>
